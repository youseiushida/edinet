...
| **過去データの蓄積** | 直近数年〜次世代EDINET以降（調整は自前） | プラン依存：ライト5年〜プレミアム約15年（株式分割等は調整済み） | 20年以上の蓄積（コーポレートアクション調整済み） | Bloomberg 30年超、FactSet/LSEG等は20年程度（ベンダー差あり、コーポレートアクション調整済み） |
| **市場データ連携** | なし（財務・非財務特化。株価等は別途取得）| あり（株価API等とIDで容易に結合可能） | あり（プラットフォーム上で完結） | 完璧（Tickデータ、コンセンサス、マクロ指標とシームレスに結合） |
| **対象カバレッジ** | 日本国内のみ（EDINET提出企業） | 日本国内のみ | 国内中心〜一部アジア・グローバル | 全世界の主要取引所上場企業を網羅 |
...
`extract_values()` で使用する正規化キー（Canonical Key = CK）が、日経225構成銘柄の有価証券報告書からどの程度抽出できるかを実測した結果です。対象は 224 社（1 社はパースエラーで除外）、全 162 CK に対する抽出成功率を示します。
...
|:---|:---|---:|
| 売上高 | `revenue` | 224/224 |
| 営業利益 | `operating_income` (\*) | 222/224 |
| 経常利益 | `ordinary_income` | 224/224 |
| 税引前利益 | `income_before_tax` | 224/224 |
| 法人税等 | `income_taxes` | 224/224 |
| 当期純利益 | `net_income` | 224/224 |
| 総資産 | `total_assets` | 224/224 |
| 負債合計 | `total_liabilities` | 224/224 |
| 純資産合計 | `net_assets` | 224/224 |
| 負債純資産合計 | `liabilities_and_net_assets` | 224/224 |
| 株主資本 | `shareholders_equity` | 224/224 |
| 資本金 | `capital_stock` | 224/224 |
| 資本剰余金 | `capital_surplus` | 224/224 |
| 利益剰余金 | `retained_earnings` | 224/224 |
| 自己株式 | `treasury_stock` | 224/224 |
| 営業CF | `operating_cf` | 224/224 |
| 投資CF | `investing_cf` | 224/224 |
| 財務CF | `financing_cf` | 224/224 |
| 現金期末残高 | `cash_end` | 224/224 |
| 1株当たり当期純利益 | `eps` | 224/224 |
| 1株当たり純資産 | `bps` | 224/224 |
| 1株当たり配当額 | `dps` | 224/224 |
| 株価収益率 | `per` | 224/224 |
| 自己資本利益率 | `roe` | 224/224 |
| 自己資本比率 | `equity_ratio` | 224/224 |
| 配当性向 | `payout_ratio` | 224/224 |
| 発行済株式総数 | `total_shares_issued` | 224/224 |
| 従業員数 | `employees` | 224/224 |
| 女性役員比率 | `female_directors_ratio` | 224/224 |
...
| 概念 | CK | 社数 | 率 |
|:---|:---|---:|---:|
| 監査報酬 | `audit_fees` | 223/224 | 99.6% |
| 繰延税金資産 | `deferred_tax_assets` | 223/224 | 99.6% |
| 親会社株主に帰属する当期純利益 | `net_income_parent` | 223/224 | 99.6% |
| 流動資産合計 | `current_assets` | 222/224 | 99.1% |
| 流動負債合計 | `current_liabilities` | 222/224 | 99.1% |
| 法人税等調整額 | `income_taxes_deferred` | 222/224 | 99.1% |
| 投資その他の資産 | `investments_and_other` | 222/224 | 99.1% |
| 固定資産合計 | `noncurrent_assets` | 222/224 | 99.1% |
| 固定負債合計 | `noncurrent_liabilities` | 222/224 | 99.1% |
| その他の包括利益累計額 | `oci_accumulated` | 222/224 | 99.1% |
| 営業利益 | `operating_income` | 222/224 | 99.1% |

### 96%〜99%未満（10 CK）

| 概念 | CK | 社数 | 率 |
|:---|:---|---:|---:|
| 現金及び預金 | `cash_and_deposits` | 221/224 | 98.7% |
| 親会社株主に係る包括利益 | `comprehensive_income_parent` | 221/224 | 98.7% |
| 潜在株式調整後EPS | `eps_diluted` | 221/224 | 98.7% |
| 無形固定資産 | `intangible_assets` | 221/224 | 98.7% |
| 中間配当額 | `interim_dps` | 221/224 | 98.7% |
| 営業外費用 | `non_operating_expenses` | 221/224 | 98.7% |
| 営業外収益 | `non_operating_income` | 221/224 | 98.7% |
| 有形固定資産 | `ppe` | 221/224 | 98.7% |
| 包括利益 | `comprehensive_income` | 220/224 | 98.2% |
| 現金及び現金同等物 | `cash_and_equivalents` | 218/224 | 97.3% |

### 90%〜96%未満（16 CK）

| 概念 | CK | 社数 | 率 |
|:---|:---|---:|---:|
| 販売費及び一般管理費 | `sga_expenses` | 215/224 | 96.0% |
| 土地 | `land` | 212/224 | 94.6% |
| 配当金の支払額（CF） | `dividends_paid_cf` | 211/224 | 94.2% |
| 為替変動による現金への影響 | `fx_effect_on_cash` | 211/224 | 94.2% |
| 投資有価証券 | `investment_securities` | 211/224 | 94.2% |
| 政策保有株式 銘柄数 | `cross_shareholdings_count` | 209/224 | 93.3% |
| 政策保有株式 貸借対照表計上額 | `cross_shareholdings_amount` | 209/224 | 93.3% |
| 支払利息（PL） | `interest_expense_pl` | 209/224 | 93.3% |
| 建設仮勘定 | `construction_in_progress` | 209/224 | 93.3% |
| 特別利益 | `extraordinary_income` | 208/224 | 92.9% |
| 特別損失 | `extraordinary_loss` | 208/224 | 92.9% |
| 有形固定資産の取得（CF） | `purchase_ppe_cf` | 207/224 | 92.4% |
| 非支配株主に係る包括利益 | `comprehensive_income_minority` | 206/224 | 92.0% |
| 非支配株主持分 | `minority_interests` | 203/224 | 90.6% |
| 非支配株主に帰属する当期純利益 | `net_income_minority` | 203/224 | 90.6% |
| 退職給付に係る負債 | `retirement_benefit_liability` | 203/224 | 90.6% |

### 80%〜90%未満（21 CK）

| 概念 | CK | 社数 | 率 |
|:---|:---|---:|---:|
| 繰延税金負債 | `deferred_tax_liabilities` | 200/224 | 89.3% |
| 営業CF 小計 | `subtotal_operating_cf` | 200/224 | 89.3% |
| 利息の支払額（CF） | `interest_expense_cf` | 199/224 | 88.8% |
| 減価償却費（CF） | `depreciation_cf` | 198/224 | 88.4% |
| 利息及び配当金の受取額（CF） | `interest_dividend_income_cf` | 198/224 | 88.4% |
| 現金及び現金同等物の増減額 | `net_change_in_cash` | 197/224 | 87.9% |
| 男女間賃金格差 | `gender_pay_gap` | 192/224 | 85.7% |
| 仕入債務の増減額（CF） | `trade_payables_change_cf` | 192/224 | 85.7% |
| 売上債権 | `trade_receivables` | 192/224 | 85.7% |
| 仕入債務 | `trade_payables` | 191/224 | 85.3% |
| 売上債権の増減額（CF） | `trade_receivables_change_cf` | 191/224 | 85.3% |
| 女性管理職比率 | `female_managers_ratio` | 190/224 | 84.8% |
| 長期借入金 | `long_term_loans` | 188/224 | 83.9% |
| 受取利息（PL） | `interest_income_pl` | 186/224 | 83.0% |
| 売上総利益 | `gross_profit` | 185/224 | 82.6% |
| 短期借入金 | `short_term_loans` | 184/224 | 82.1% |
| 減損損失（PL） | `impairment_loss_pl` | 184/224 | 82.1% |
| 減価償却費（販管費） | `depreciation_sga` | 183/224 | 81.7% |
| 売上原価 | `cost_of_sales` | 181/224 | 80.8% |
| 建物純額 | `buildings_net` | 181/224 | 80.8% |
| 棚卸資産の増減額（CF） | `inventories_change_cf` | 180/224 | 80.4% |

### 50%〜80%未満（20 CK）

| 概念 | CK | 社数 | 率 |
|:---|:---|---:|---:|
| 男性育児休業取得率 | `male_childcare_leave_rate` | 179/224 | 79.9% |
| 有形固定資産の売却収入（CF） | `proceeds_ppe_sale_cf` | 178/224 | 79.5% |
| 研究開発費 | `rd_expenses` | 174/224 | 77.7% |
| 社債 | `bonds_payable` | 172/224 | 76.8% |
| 持分法による投資損益（CF） | `equity_method_cf` | 172/224 | 76.8% |
| のれん | `goodwill` | 171/224 | 76.3% |
| 減損損失（CF） | `impairment_loss_cf` | 168/224 | 75.0% |
| 長期借入金の返済（CF） | `repayment_long_term_loans_cf` | 154/224 | 68.8% |
| 自己株式の取得（CF） | `purchase_treasury_stock_cf` | 150/224 | 67.0% |
| 長期借入れによる収入（CF） | `proceeds_long_term_loans_cf` | 142/224 | 63.4% |
| 契約負債 | `contract_liabilities` | 139/224 | 62.1% |
| 前払費用 | `prepaid_expenses` | 137/224 | 61.2% |
| 受取手形 | `notes_receivable` | 126/224 | 56.2% |
| その他（営業CF） | `other_operating_cf` | 124/224 | 55.4% |
| 法人税等の支払額（CF） | `income_taxes_paid_cf` | 122/224 | 54.5% |
| 1年内償還社債 | `current_portion_of_bonds` | 122/224 | 54.5% |
| 有形固定資産売却損益（CF） | `ppe_sale_loss_gain_cf` | 119/224 | 53.1% |
| 棚卸資産 | `inventories` | 118/224 | 52.7% |
| その他（投資CF） | `other_investing_cf` | 113/224 | 50.4% |
| 投資有価証券の取得（CF） | `purchase_investment_securities_cf` | 112/224 | 50.0% |
...
| 概念 | CK | 社数 | 率 |
|:---|:---|---:|---:|
| 社債の償還（CF） | `redemption_bonds_cf` | 111/224 | 49.6% |
| 投資有価証券の売却収入（CF） | `proceeds_investment_securities_cf` | 104/224 | 46.4% |
| 1年内返済長期借入金 | `current_portion_of_long_term_loans` | 104/224 | 46.4% |
| その他（財務CF） | `other_financing_cf` | 99/224 | 44.2% |
| 新株予約権 | `subscription_rights` | 99/224 | 44.2% |
| 持分法による投資利益（PL） | `equity_method_income` | 97/224 | 43.3% |
| 設備投資額 | `capex` | 96/224 | 42.9% |
| 親会社の所有者に帰属する持分 | `equity_parent` | 94/224 | 42.0% |
| 持分法による投資利益（IFRS） | `equity_method_income_ifrs` | 85/224 | 37.9% |
| 持分法で会計処理されている投資 | `equity_method_investments` | 85/224 | 37.9% |
| 金融費用 | `finance_costs` | 85/224 | 37.9% |
| 社債の発行による収入（CF） | `proceeds_bonds_cf` | 85/224 | 37.9% |
| 金融収益 | `finance_income` | 83/224 | 37.1% |
| 引当金（流動） | `provisions_cl` | 75/224 | 33.5% |
| 貸付金の回収（CF） | `loans_paid_cf` | 73/224 | 32.6% |
| 引当金（固定） | `provisions_ncl` | 73/224 | 32.6% |
| 貸倒引当金の増減額（CF） | `allowance_doubtful_change_cf` | 71/224 | 31.7% |
...
| 概念 | CK | 社数 | 率 | 備考 |
|:---|:---|---:|---:|:---|
| その他の収益（IFRS） | `other_income_ifrs` | 67/224 | 29.9% | IFRS |
| 契約資産 | `contract_assets` | 67/224 | 29.9% | |
| 短期借入金の純増減額（CF） | `short_term_borrowings_net_cf` | 66/224 | 29.5% | |
| その他の費用（IFRS） | `other_expenses_ifrs` | 65/224 | 29.0% | IFRS |
| 為替差損益（CF） | `fx_loss_gain_cf` | 62/224 | 27.7% | |
| 使用権資産 | `right_of_use_assets` | 61/224 | 27.2% | IFRS |
| 非支配株主への配当金の支払額（CF） | `dividends_paid_nci_cf` | 59/224 | 26.3% | |
| のれん償却額（CF） | `goodwill_amortization_cf` | 59/224 | 26.3% | J-GAAP のみ |
| 貸付金の回収による収入（CF） | `loans_collected_cf` | 56/224 | 25.0% | |
| 支払手形 | `notes_payable` | 54/224 | 24.1% | |
| 有利子負債（固定） | `interest_bearing_debt_ncl` | 52/224 | 23.2% | |
| 棚卸資産の評価損（CF） | `inventory_writedowns` | 52/224 | 23.2% | |
| 有利子負債（流動） | `interest_bearing_debt_cl` | 49/224 | 21.9% | |
| 配当金の受取額（CF） | `dividends_received_cf` | 46/224 | 20.5% | |
| 連結範囲の変動を伴う子会社株式の取得・売却 | `consolidation_scope_change_cash` | 40/224 | 17.9% | |
| リース負債（流動） | `lease_liabilities_cl` | 38/224 | 17.0% | IFRS |
| リース負債（非流動） | `lease_liabilities_ncl` | 38/224 | 17.0% | IFRS |
| 受取配当金 | `dividend_income` | 28/224 | 12.5% | |
| 非支配株主からの払込による収入（CF） | `nci_capital_contribution_cf` | 23/224 | 10.3% | |
| 繰延資産 | `deferred_assets` | 22/224 | 9.8% | |
| 投資不動産 | `investment_property` | 21/224 | 9.4% | |
| 販売用不動産 | `real_estate_for_sale` | 17/224 | 7.6% | |
| 預金（銀行業） | `deposits` | 10/224 | 4.5% | 銀行業 |
| 継続事業からの利益 | `continuing_operations_income` | 5/224 | 2.2% | US-GAAP |
| 正味収入保険料 | `net_premiums_written` | 3/224 | 1.3% | 保険業 |
| 有利子負債合計 | `interest_bearing_debt` | 2/224 | 0.9% | |
| 貸出金 | `loans_and_bills_discounted` | 2/224 | 0.9% | 銀行業 |
| 株式報酬費用（CF） | `sbc_cf` | 2/224 | 0.9% | |
| 有価証券（銀行業） | `securities_banking` | 2/224 | 0.9% | 銀行業 |
| 自己資本比率（BIS基準） | `capital_adequacy_ratio_bis` | 0/224 | 0.0% | 銀行業（BIS） |
| 自己資本比率（国内基準） | `capital_adequacy_ratio_domestic` | 0/224 | 0.0% | 銀行業（国内） |
| 自己資本比率（国内基準2） | `capital_adequacy_ratio_domestic_2` | 0/224 | 0.0% | 銀行業（国内2） |
| 自己資本比率（国際統一基準） | `capital_adequacy_ratio_international` | 0/224 | 0.0% | 銀行業（国際） |
| 利息及び配当金収入（保険業） | `interest_dividend_income_ins` | 0/224 | 0.0% | 保険業 |
| 資産運用収益 | `investment_yield_income` | 0/224 | 0.0% | 保険業 |
| 資産運用等の実現利益 | `investment_yield_realized` | 0/224 | 0.0% | 保険業 |
| リース負債 | `lease_liabilities` | 0/224 | 0.0% | IFRS |
| 正味損害率 | `net_loss_ratio` | 0/224 | 0.0% | 保険業 |
| 正味事業費率 | `net_operating_expense_ratio` | 0/224 | 0.0% | 保険業 |
...
| **90%〜96%** | 16 | 65 | 特別損益 + 少数株主 + BS実物資産 |
| **80%〜90%** | 21 | 86 | CF明細 + ESG + 借入金 + 減損・減価償却 |
| **50%〜80%** | 20 | 106 | のれん + 投資CF明細 + 契約負債・手形 |
| **30%〜50%** | 17 | 123 | IFRS科目 + 持分法 + 引当金 |
| **30%未満** | 39 | 162 | 業種限定（銀行・保険）+ 使用権資産 |
