...
| 5 | 非同期 HTTP (asyncio) | 大量処理で 10x 改善 | v0.2.0 以降 |
| x | パーサーを C/Rust で書く | 全体の数%しか改善しない | 不要 |

httpx を採用する理由: 同期/非同期の両方をサポートしており、最初は同期で書いて後から async 化が可能。requests や urllib ではこれができない。
...
markers = [
    ...
    "medium: requires local fixture files",
]
...
| 1' | 様式コードリスト | [ESE140327.xlsx](https://disclosure2dl.edinet-fsa.go.jp/guide/static/disclosure/download/ESE140327.xlsx) | Excel全行 | 全41型式の公式定義 |
...
- 様式コードリスト (ESE140327.xlsx) で全41型式を確認
...
- `models/doc_types.py`: 全41型式を Enum 定義（情報源は様式コードリスト Excel の1つだけ）
...
- [注意] get_filings() の日付ループを独立した関数に切り出す（将来 async 化のため）
...
- `tests/test_xbrl/test_parser.py`: Small テスト
...
- タクソノミ概要説明 (ESE140108.pdf) を読む
- 勘定科目リスト (ESE140115.xlsx) を Excel で開く
...
- [パフォーマンス] キャッシュパス: `platformdirs.user_cache_dir("edinet") / "taxonomy_labels_v{__version__}.pkl"`（Linux: `~/.cache/edinet/`, macOS: `~/Library/Caches/edinet/`, Windows: `AppData\Local\edinet\Cache\`）。バージョンキーを含めることで、クラス定義変更時の pickle デシリアライズ失敗を防止。ロード失敗時は警告を出して再構築する
...
**変更:** `LineItem` に `context_id: str` と `dimensions: dict[str, str]` を追加
